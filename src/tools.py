...
class tools:
    ...
    def save_subject_voting_proportions(self, all_fold_results):
        ...
        df_summary.to_csv(output_path, index=False, float_format='%.1f')
        ...
        return df_summary
    #================================================================================================ 
    def report_performance_separately(self, df_summary, output_path=None):
        df = df_summary.copy()
    
        df['sub_num'] = df['sub'].apply(
            lambda x: int(str(x).split('_')[-1]) if '_' in str(x) else int(x)
        )
    
        total_count = len(df)
        max_sub = df['sub_num'].max()
        ds =0
        if total_count == 65:
            df['Dataset'] = 'Dataset 1'
            datasets_to_process = ['Dataset 1']
            ds = 1
        elif total_count == 67:
            df['Dataset'] = 'Dataset 2'
            datasets_to_process = ['Dataset 2']
            ds= 2
        else:
            ds=12
            df['Dataset'] = df['sub_num'].apply(
                lambda n: 'Dataset 1' if n <= 65 else 'Dataset 2'
            )
            datasets_to_process = ['Dataset 1', 'Dataset 2']
    
        def assign_ground_truth(row):
            n = row['sub_num']
            if total_count == 67 and max_sub <= 67:
                return 'AD' if n <= 35 else 'HC'
        ...
        df['True_Label'] = df.apply(assign_ground_truth, axis=1)
        if ds == 1:
             df['Pred_Label'] = df.apply(
            lambda row: 'AD' if round(row['AD'], 1) >= round(row['HC'], 1) else 'HC', axis=1
        )
        elif ds == 2 or ds == 12:
            df['Pred_Label'] = df.apply(
            lambda row: 'AD' if round(row['AD'], 1) > round(row['HC'], 1) else 'HC', axis=1
        ...
        def calculate_metrics(sub_df):
            tp = sum((sub_df['True_Label'] == 'AD') & (sub_df['Pred_Label'] == 'AD'))
            tn = sum((sub_df['True_Label'] == 'HC') & (sub_df['Pred_Label'] == 'HC'))
            fp = sum((sub_df['True_Label'] == 'HC') & (sub_df['Pred_Label'] == 'AD'))
            fn = sum((sub_df['True_Label'] == 'AD') & (sub_df['Pred_Label'] == 'HC'))
        ...
        if output_path is None and hasattr(self, 'paths') and 'tables' in self.paths:
            output_path = self.paths['tables'] / 'report_performance_separately.csv'
    
        if output_path:
            report_df.to_csv(output_path, index=False)
            print(f'Stratified performance report saved successfully to: {output_path}')
