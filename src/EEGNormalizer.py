    ...
    def _normalize_per_sample(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            if self.method == 'minmax':
                ...
                scale = np.maximum(sample_max - sample_min, self.EPS)
                sample = (sample - sample_min) / scale
    
            elif self.method == 'zscore':
                center = sample.mean(axis=1, keepdims=True)
                scale = np.maximum(sample.std(axis=1, keepdims=True), self.EPS)
                sample = (sample - center) / scale
    
            elif self.method == 'zscore_robust':
                center = np.median(sample, axis=1, keepdims=True)
                mad = np.median(np.abs(sample - center), axis=1, keepdims=True)
                scale = np.maximum(1.4826 * mad, self.EPS)
                sample = (sample - center) / scale
    
            elif self.method == 'winsor_zscore':
                q_low = np.quantile(sample, 0.01, axis=1, keepdims=True)
                q_high = np.quantile(sample, 0.99, axis=1, keepdims=True)
                sample = np.clip(sample, q_low, q_high)
                center = sample.mean(axis=1, keepdims=True)
                scale = np.maximum(sample.std(axis=1, keepdims=True), self.EPS)
                sample = (sample - center) / scale
            ...
            # تطبيق clip_sigma إذا كان محدداً
            if self.clip_sigma is not None and self.clip_sigma > 0:
                sample = np.clip(sample, -self.clip_sigma, self.clip_sigma)
